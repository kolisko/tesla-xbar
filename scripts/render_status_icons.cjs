...
const drawings = Object.fromEntries([...names, 'api-error'].map(name => [name,
...
async function main() {
  // A read failure replaces live indicators, so it needs only one standalone asset.
  const errorPng = await sharp(Buffer.from(svg(16, 16, drawings['api-error'])), {density: 144}).png().toBuffer();
  fs.writeFileSync(path.join(directory, 'api-error.png'), retinaDensity(errorPng));
  ...
  const preview = svg(940, 460, `<rect width="940" height="460" rx="20" fill="#202932"/>
    <g font-family="Arial, sans-serif" fill="#e6edf3">
    <text x="28" y="36" font-size="19" font-weight="bold">Vehicle indicators</text>
    ...
    <rect x="28" y="336" width="180" height="52" rx="12" fill="#12191f"/>
    <g transform="translate(46 353)">${drawings['api-error'].replaceAll('#000', '#e6edf3')}</g>
    <text x="77" y="371" font-size="24">— km</text>
    <text x="230" y="369" font-size="17">Three failed refreshes: current value unavailable</text>
    <text x="28" y="433" font-size="14" fill="#a0a6ad">Illustrative data. Active indicators require a fresh, online reading. Saved readings remain in the menu.</text></g>`);
