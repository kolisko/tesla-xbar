...
const names = ['charging', 'camp', 'pet', 'fan', 'unlocked', 'sentry'];
...
async function main() {
    ...
    for (const mode of [null, 'camp', 'pet']) {
      for (const fan of [false, true]) {
        for (const unlocked of [false, true]) {
         for (const sentry of [false, true]) {
          const icons = [charging && 'charging', mode, fan && 'fan', unlocked && 'unlocked', sentry && 'sentry'].filter(Boolean);
  ...
  const preview = svg(1120, 260, `<rect width="1120" height="260" rx="20" fill="#202932"/>
    ...
    ${names.map((name, i) => `<g transform="translate(${34 + i * 183} 65) scale(2)" fill="#e6edf3">${drawings[name].replaceAll('#000', '#e6edf3')}</g>
      <text x="${28 + i * 183}" y="126" font-size="17">${['Charging', 'Camp Mode', 'Pet Mode', 'Climate on', 'Unlocked', 'Sentry'][i]}</text>`).join('')}
