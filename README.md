...
- **Debug info:** the latest two plugin runs and latest two dispatched Tesla API requests, with local millisecond timestamps, plus the number of consecutive failed refreshes. Paused runs advance only the plugin times; error responses still count as API requests.
...
- The first two failed refreshes keep the saved value and cable/range color, with the error visible in the menu. The third consecutive failure replaces the value and color with a neutral placeholder and warning icon. This is a display warning, not a billing counter or a change to polling frequency. A successful availability check reporting offline/asleep restores the ordinary saved-value display; an online car must also provide a valid battery reading to clear the warning.
