...
- **One refresh schedule:** new installs default to five minutes (`5m`); xBar's filename controls routine polling. The plugin's **Refresh now** explicitly attempts API access immediately, bypassing local error backoff but honoring Tesla's `Retry-After`. Built-in xBar refresh/Refresh All still obey backoff. No resident retry timer or invented monthly quota.
...
- **Debug info:** the latest two plugin runs and latest two dispatched Tesla API requests, with local millisecond timestamps, plus the number of consecutive failed refreshes and the next automatic retry's earliest time. Recorded Tesla `Retry-After` advice shows its original value, receipt time, deadline and whether it is still active; the menu identifies whether Tesla or local backoff controls the next attempt. The request runs on the first visible xBar invocation at or after that time. Paused runs advance only the plugin times; error responses still count as API requests.
...
Requires macOS, xBar, Python 3.9+, Go 1.23+, OpenSSL, Git and Apple's Command Line Tools. The installer builds the Swift Keychain, address, map-image and desktop-visibility helpers and Tesla's official command helper from a pinned source revision.
...
See the [API usage review](docs/API_USAGE.md) for requests per refresh, volume estimates and proposed optimizations.
...
| Refresh interval | Managed by xBar's plugin filename: the default `tesla-battery.5m.sh` runs every five minutes. Existing installations keep their chosen interval when updated. Change it through xBar's plugin management. There is no separate interval in `config.json`. |
...
- The first two failed refreshes keep the saved value and cable/range color, with the error visible in the menu. The third consecutive failure replaces the value and color with a neutral placeholder and warning icon and starts the 15/30/60-minute backoff. A successful availability check reporting offline/asleep restores the ordinary saved-value display and normal polling; an online car must also provide a valid battery reading to clear the warning. Renewed account consent clears local backoff, but does not itself clear the warning or invent a fresh reading; unexpired Tesla retry advice still applies.
