...
Use pull requests for changes to `main`; required checks must pass. GitHub Actions are pinned to commit SHAs. The official Tesla SDK revision is pinned in [`scripts/build_commands.py`](../scripts/build_commands.py); a version update must include review of upstream changes and a passing Go vulnerability audit. Dependabot updates workflow actions, not the SDK's embedded revision.
