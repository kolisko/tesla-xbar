...
3. A normal refresh first checks the injected `Desktop` port. If the desktop is hidden or visibility cannot be verified, it returns saved readings without constructing the Tesla gateway, renewing tokens or running location/map lookups. Otherwise it checks the persisted retry deadline before constructing a gateway, then gets vehicle availability and, when online, one combined reading when due. An unavailable car retains its last reading. Normal refresh never calls `execute`.
...
New installations use `tesla-battery.5m.sh`; updates preserve the existing filename. xBar remains the only scheduler. Every routine `menu` invocation checks `DesktopVisibility` before any Tesla or map work. The plugin’s explicit **Refresh now** uses a separate `refresh` request. The native probe has a three-second timeout; missing, malformed or failed probe results pause polling rather than silently sending requests.
...
Paused output keeps the saved range/percentage, cable color and original reading times, labels statuses as last known and suppresses live icons/pulsing. If a repeated-read-error warning is already active, that warning and placeholder remain instead. The pause reason exists only in the current result, never as a fabricated vehicle state or new cache timestamp. The next visible invocation resumes immediately; there is no wake observer or extra polling loop. A request already in flight when the screen goes dark may finish. Explicit account actions and physical commands remain user initiated and are not cancelled if visibility changes after a click.

## Read failure state

`VehicleService` persists `consecutive_read_errors` in the selected vehicle's private state record, under the existing operation lock. Each failed availability/live refresh attempt increments it once, regardless of internal authentication retries or location fallback. A recovered location fallback is a successful refresh. Selection/configuration errors and map/address work are not counted as Tesla read failures. A successful complete refresh resets the count: an online vehicle needs a valid battery reading, while a successful offline/asleep availability response is sufficient. An unavailable-vehicle response (mapped from HTTP 408 at the adapter boundary) also returns to the normal offline display. Visibility pauses, concurrent-operation skips and server-directed backoff leave the count unchanged. Renewed consent alone does not prove that vehicle reading works.

The domain activates a warning at three failures, suppresses cable/range colors and live status icons, and chooses the standalone `api-error` template image. The pure menu renderer replaces the top value with a unit-aware dash and describes the saved readings below. Last successful values and their timestamps remain intact. Debug info exposes the failure count. The indicator itself adds no requests or timer; retry timing follows the policy below.


## Retry backoff and explicit refresh

The domain's `polling.py` chooses 15 minutes after the third failed refresh, 30 after the fourth, and 60 after every subsequent failure. `VehicleService` saves an absolute `read_retry_at` after a failed attempt finishes. The effective deadline is the later of that value and Tesla's rate-limit `retry_at`. Early xBar runs return saved state before constructing a gateway, token renewal or map/address work; they neither increment the failure count nor move the deadline. The existing private operation lock serializes attempts. No sleeping worker or second scheduler exists. A successful refresh or normal unavailable-vehicle result clears the error deadline; renewed consent clears local backoff but retains the warning until a successful vehicle check. Actual unexpired server advice remains binding.

The plugin's **Refresh now** dispatches `refresh`, while the wrapper always dispatches `menu` for scheduled and built-in xBar refreshes. The explicit action bypasses desktop gating and local error backoff, but still respects a server-directed `Retry-After`; it never wakes the vehicle. A failed manual attempt advances the error series and schedules the next automatic attempt. After completion, `PluginService` saves `manual_refresh_completed_at`: ordinary menu invocations within five seconds reuse this result so xBar's immediate `refresh=true` redraw does not duplicate the read. Another explicit click always attempts again. This short result-reuse window does not alter xBar's normal interval or delay recovery for a whole scheduled cycle.

The Python HTTP adapter attaches typed `RetryAdvice` to surfaced remote errors: the actual Retry-After value (bounded to 256 characters), its receipt timestamp and a parsed seconds/date delay. `VehicleService` retains the latest advice in the private state record as `tesla_retry_after`, including after expiry or recovery. Missing headers do not erase earlier evidence; invalid values have no computed server deadline. The policy honors the receipt-based deadline even for non-429 errors and explicit manual refresh. A 401 renewal retry or location-permission fallback cannot immediately bypass valid server advice. Debug info renders the raw value safely, receipt time, waiting/elapsed deadline and the policy responsible for the final retry time. No other response headers, body, tokens or identifiers are stored by this feature.

Debug info shows **Next automatic API attempt** using the effective retry deadline in local time. It is a not-before time: the actual attempt waits for the first visible xBar invocation at or after it. When there is no future retry deadline, the menu says the next visible run will attempt, or that the desktop must become visible again. No exact future dispatch time is fabricated while xBar or the desktop is paused.

## Local request diagnostics

The infrastructure diagnostics adapter stores just two timestamps for each of `plugin_runs` and `tesla_api_calls` in the private profile's `diagnostics.json`. The composition root records `menu`/`refresh` entry before the visibility check. The HTTP adapter records a Tesla request once it has been written; proxy requests are recorded when a response (including HTTP errors) confirms dispatch. OAuth, availability and live reads are included. A local cache read, visibility probe or failed connection before dispatch does not advance API times. Proxy requests that fail without any response cannot be confirmed as dispatched.

The small Go diagnostics adapter wraps Tesla's SDK transport with `httptrace.WroteRequest`, so signed commands, session negotiation and SDK retries also count when actually written. It leaves command semantics and SDK code unchanged. Both languages use the same short file lock and atomic replacement; the profile's main operation lock is separate. Diagnostic failures never replay or fail a vehicle command. The pure menu renderer receives a snapshot through `MenuContext`. Opening **Debug info** adds no API call; times use the Mac's local timezone with milliseconds, and missing history is explicitly labeled. This is recent activity information, not a billing meter or a historical usage log.
