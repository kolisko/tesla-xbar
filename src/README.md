...
| [`visibility.swift`](visibility.swift) | Source for `tesla-visibility`: read-only desktop/menu-bar availability probe with no network, permissions prompts or resident process. |
...
The installer also generates `tesla-action.sh` and the `tesla-battery.5m.sh` xBar entry point with local executable paths. Tesla's Go command helper is built separately by [`scripts/build_commands.py`](../scripts/build_commands.py).
